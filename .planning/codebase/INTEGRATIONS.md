...
- **Client:** `neo4j` Python driver (≥ 5.14), singleton in [core/connections.py](core/connections.py)
...
- **Client:** `qdrant-client` (≥ 1.8), singleton in [core/connections.py](core/connections.py)
...
- **Role:** read-only at query time; only written during step 8 of ETL

### AWS DynamoDB (source data, read-only)
- **Client:** `boto3` (≥ 1.34)
- **Tables:** `DefaultPlattersTable`, `DefaultPlatterItemsTable`, `DefaultPlattersCategoriesTable`
...
- **Called in:**
  - `scripts/index_communities.py` — embed community summaries (batched)
  - `scripts/search.py` — embed user query items at query time (batched)
- **No chat completions used**

### Google Gemini (LLM reasoning)
- **Client:** `google-genai` (≥ 1.0)
- **Used for:**
  - `scripts/enrich_items.py` — structured item descriptions (ingredients, form, regional tags)
...
- **Retry config:** `MAX_RETRIES=3`, exponential backoff, logged per attempt
- **Caching:** on-disk in `llm_cache/enrichment/` and `llm_cache/variants/`, keyed by canonical ID + prompt version hash
...
- `llm_cache/enrichment/` — `enrich_items.py` results
...
- **Streamlit UI** ([app.py](app.py)) calls into `scripts/search.py` directly (no HTTP boundary)
...
---
*Integrations analysis: 2026-04-15*
